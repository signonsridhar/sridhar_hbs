define(['bases/control', 'models/country_list/country_list',], (BaseControl, CountryList)->
    BaseControl.extend({
        COUNTRIES : [
            {
                "countryCode": "AF",
        ...
        init:(elem, options)->
            ###this.countries = options.countries = new CountryList()###
            INPUT = this.constructor.INPUT
            this.options.mode = this.constructor.INPUT.COUNTRY_ABBR if not options.mode?

            switch this.options.mode
                when INPUT.COUNTRY_ABBR
                    mode_key = 'abbr'
                when INPUT.COUNTRY_PHONE_CODE
                    mode_key = 'countryPhoneNumberCode'
                else
                    mode_key = 'countryName'

            this.render('country_dropdown/country_dropdown', {
                attr: this.options.attr,
                mode_key:mode_key ,
                COUNTRIES: this.constructor.COUNTRIES
            })

            this.find('option[value="ext"]').remove() if options.no_ext
            cntry_code = this.options.model.attr('country_code')
            #TODO HACK why is init called twice
            setTimeout(()=>
                return unless this.element
                #TODO HACK find('select') is setting this.options.model.attr('country_code')) to null(for Ext ??)
                # and invoking {model} change
                select = this.find('select')
                if cntry_code == 'ext'
                    select.find("option:first-child").attr("selected", "selected")
                else
                    select_option = select.find('[data-cntry_prefix="' + this.options.model.attr('country_prefix') + '"]')
                    console.log("select option", select_option.html())
                    select_option.attr('selected', 'selected')
            , 10 )

            CountryList.load().done (response)=>
                #console.log(' country list response', response)

        '{model} change':()->
            #console.log('country code ', this.options.model.country_code)
            this.options.model.attr('country_prefix', this.element.find("option:selected").data('cntry_prefix'))
